...
describe('temporary repository support', () => {
  it('initializes main with a local identity, creates explicit commits, and removes the temporary repository', async () => {
    ...
    try {
      ...
      expect(
        runGit({ arguments: ['config', 'user.name'], cwd: repository.path }),
      ).toBe('Pi Maestro Test');
      expect(
        runGit({ arguments: ['config', 'user.email'], cwd: repository.path }),
      ).toBe('pi-maestro-test@example.com');
      expect(
        runGit({ arguments: ['rev-parse', 'HEAD'], cwd: repository.path }),
      ).toBe(commit);
    } finally {
