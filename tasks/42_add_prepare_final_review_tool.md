...
5. Clearly state in structured data that Maestro is concluded and the owner controls later edits and the final commit.
6. Reject repeated preparation of an already concluded workflow.
...
Add Vitest adapter tests for a no-finding candidate, rejected findings candidate, dirty base, stale revision, failed staging verification, cleanup failure handling, successful result fields, inactive mode, repeated call, and proof that no final commit exists.
