...
4. Stage product changes and `workflow.json` in `final-review`.
5. Verify the exact expected staging before cleanup.
6. Remove verified workflow worktrees and branches immediately after staging succeeds.
...
Use rollback or stop-before-cleanup behavior so a failed squash or staging check does not destroy workflow resources. Never include unrelated owner changes. After successful cleanup, later owner edits are outside Maestro.

## Tests

Add Vitest integration tests for each candidate type, squash contents, staged final state, no final commit, observations data, branch and worktree cleanup, parent-directory cleanup, dirty base rejection, unexpected staging, failure before cleanup, and completed-workflow discovery.
